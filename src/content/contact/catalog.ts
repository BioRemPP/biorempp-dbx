...
import { assertNonEmptyString, assertNonEmptyStringArray } from '../shared/guards';
import rawContactCatalog from '../editorial/pages/contact.page.yaml?raw';
import { resolveContactImage } from './assets';
import type {
...
export function parseContactCatalog(rawYaml: string): ContactPageCatalog {
  let parsed: unknown;
  try {
  ...
  return {
    ...
    laboratory: {
      ...
      paragraphs: assertNonEmptyStringArray(labObject.paragraphs, 'laboratory.paragraphs'),
...
export function loadContactCatalog(): ContactPageCatalog {
  return parseContactCatalog(rawContactCatalog);
}
