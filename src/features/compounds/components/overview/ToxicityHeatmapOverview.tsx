 ...
 *
 * Toxicity heatmap overview for compound detail pages, grouping endpoint predictions and risk
 * buckets into domain-oriented facets.
 */
import { ChartCard } from '@shared/visualization/charts/ChartCard';
import type { ToxicityHeatmapDatum } from '@/features/toxicity/types';
import { riskBucketLabel, riskBucketToScore } from '@features/compounds/utils/overviewAdapters';
...
/**
 * Props accepted by the toxicity heatmap overview.
 */
interface ToxicityHeatmapOverviewProps {
  /** Endpoint-level overview rows returned by the compound overview endpoint. */
...
/**
 * Renders a grouped toxicity heatmap with prediction and categorical risk rows.
 *
 * @param props Endpoint overview rows for the selected compound.
 * @returns A chart card containing a grouped heatmap table or an empty-state message.
 *
 * @remarks
 * Endpoint rows are first normalized into toxicity facets, then flattened so grouped headers,
 * prediction cells, and risk cells stay aligned across the final heatmap table.
 */
