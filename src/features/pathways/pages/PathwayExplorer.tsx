 ...
 *
 * Pathway explorer page with source-scoped rankings, paginated search results, and optional
 * navigation into pathway detail views.
 */
...
/**
 * Props accepted by the pathway explorer page.
 */
interface PathwayExplorerProps {
  /** Optional callback invoked when a result row opens a pathway detail view. */
...
/**
 * Renders the pathway explorer with source selection, search, and paginated rankings.
 *
 * @param props Optional pathway-selection callback used by interactive table rows.
 * @returns The pathway explorer layout with source controls, summary state, and the result table.
 *
 * @remarks
 * The explorer keeps one source in focus at a time by default, using `KEGG` as the initial source
 * filter until the user explicitly changes it.
 */
export function PathwayExplorer({ onPathwaySelect }: PathwayExplorerProps) {
  ...
  return (
    <ExplorerLayout
      ...
      toolbar={
        <FilterToolbar>
          <SearchField value={searchInput} onChange={setSearchInput} onSearch={handleSearch} placeholder="e.g. Benzoate degradation" />
        </FilterToolbar>
      }
      filters={
