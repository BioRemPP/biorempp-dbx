 ...
 *
 * Filter-control renderer for guided analysis queries, covering search, select, range, and toggle
 * field types.
 */
import type { GuidedFilterDefinition, GuidedFilterOption, GuidedFilterState, GuidedFilterValue } from '@/features/guided-analysis/types';

/**
 * Props accepted by the guided filters bar.
 */
interface GuidedFiltersBarProps {
  /** Filter definitions declared by the selected guided query. */
    ...
    <div key={filter.id} className="w-full">
      ...
      <div className="grid grid-cols-2 gap-2">
        <input
          ...
          placeholder={filter.min_placeholder || 'Min'}
          className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm"
        />
        <input
          id={maxId}
          ...
          placeholder={filter.max_placeholder || 'Max'}
          className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm"
        />
      </div>
    </div>
...
/**
 * Renders guided filter controls for the selected query.
 *
 * @param props Filter definitions, current values, option sets, and callbacks.
 * @returns A responsive filter section, or `null` when the query declares no filters.
 */
