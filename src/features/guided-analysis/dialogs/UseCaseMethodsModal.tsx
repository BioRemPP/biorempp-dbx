 ...
 *
 * Modal dialog for rendering the methods narrative attached to a guided analysis use case.
 */
import { useEffect, useId, useState } from 'react';
import { X } from 'lucide-react';
import type { GuidedMethodsModal } from '@/features/guided-analysis/types';

/**
 * Props accepted by the guided methods modal.
 */
interface UseCaseMethodsModalProps {
  /** Methods modal content declared by the selected guided query. */
  content: GuidedMethodsModal;
  /** Optional override used by UI-specific variants without mutating the content contract. */
  buttonLabelOverride?: string;
}

/**
 * Renders a dialog that explains the analytical workflow for a guided use case.
 *
 * @param props Methods-modal content declared by the selected query.
 * @returns A button that opens the methods dialog plus the dialog content when open.
 *
 * @remarks
 * The dialog closes on backdrop click, explicit close buttons, and the `Escape` key while open.
 */
export function UseCaseMethodsModal({ content, buttonLabelOverride }: UseCaseMethodsModalProps) {
