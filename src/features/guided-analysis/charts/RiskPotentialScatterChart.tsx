 ...
 *
 * Scatter chart for guided-analysis use cases that compare annotation-derived potential against
 * toxicity-oriented risk metrics.
 */
...
/**
 * Props accepted by the risk-versus-potential scatter chart.
 */
interface RiskPotentialScatterChartProps {
  /** Scatter points returned by the guided execution payload. */
  points: GuidedScatterPoint[];
  /** Threshold applied to the x axis for quadrant overlays. */
  xThreshold: number;
  /** Threshold applied to the y axis for quadrant overlays. */
  yThreshold: number;
  /** Scale mode used to interpret and label the x axis. */
  xScaleMode?: 'log10p1' | 'linear';
  /** Human-readable label shown on the y axis. */
  yMetricLabel?: string;
  /** Callback invoked when a scatter point opens a compound detail view. */
  onSelectCompound: (cpd: string) => void;
}

/**
 * Renders the guided scatter visualization for risk-versus-potential use cases.
 *
 * @param props Scatter points, thresholds, axis labels, and drill-down callback.
 * @returns A scatter chart with thresholds, point sizing, and class legend.
 */
